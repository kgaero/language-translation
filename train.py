...
def greedy_decode(model, source, source_mask, tokenizer_tgt, max_len, device):
    ...
    while decoder_input.size(1) < max_len:
        ...
        decoder_input = torch.cat(
            [decoder_input, torch.full((1, 1), next_word.item(), dtype=source.dtype, device=device)],
            dim=1,
        )
...
def run_validation(
    ...
    if writer and torchmetrics and predicted:
...
def _is_within_length(item, tokenizer_src, tokenizer_tgt, config):
...
def get_ds(config):
    ...
    filtered_samples = [item for item in ds_raw if _is_within_length(item, tokenizer_src, tokenizer_tgt, config)]
    if not filtered_samples:
        raise RuntimeError("No samples fit the configured seq_len. Increase seq_len or use shorter data.")

    train_ds_size = int(0.9 * len(filtered_samples))
    val_ds_size = len(filtered_samples) - train_ds_size
    train_ds_raw, val_ds_raw = random_split(filtered_samples, [train_ds_size, val_ds_size])
